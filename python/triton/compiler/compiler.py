...
def parse(full_name, ext, context):
    ...
    if ext == "cubin" or ext == "hsaco" or ext == "blob":
...
def compile(src, target=None, options=None, _env_vars=None):
    ...
    for ext, compile_ir in list(stages.items())[first_stage:]:
        ...
        # If TRITON_STORE_BINARY_ONLY is 1, only store binary artifacts + json
        if (not store_only_binary) or (ext in ("cubin", "hsaco", "blob", "json")):
